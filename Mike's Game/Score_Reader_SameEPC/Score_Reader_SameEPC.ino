...
void loop()
{
  ...
  while (responseType != RESPONSE_SUCCESS)//RESPONSE_IS_TAGFOUND)
  {
    ...
    responseType = nano.readTagEPC(myEPC, myEPClength, 2000); //Scan for a new tag up to 500ms
...
void UpdateRedScore(int rc){
  if (rc > 5){
    Redscore++;
    Serial.println("Red Score: " + String(Redscore));
  }
}
...
void UpdateBlueScore(int bc){
  if (bc > 5){
    Bluescore++;
    Serial.println("Blue Score: " + String(Bluescore));
  }
}
