...
const double WAYPT_LAT_LNG[NUM_WAYPTS][2] = {
  {44.280326, -71.691144},      // WMSI
  {44.281956, -71.691390}        //Park Avenue 
};

const String bearings[NUM_WAYPTS] = {
  "3 * 5",
  "22 + 23"
};
...
void loop() {
  ...
  if (gps.location.lat() - gps.location.rawLat().deg > 0) { 
    Raw_Lat += gps.location.rawLat().deg;
  }
  else {
      Raw_Lat += (gps.location.rawLat().deg -1); //because the gps rounds to degreee, checks to see if rounded up or not
  }
  if (gps.location.lng() - gps.location.rawLng().deg > 0){
      Raw_Long += gps.location.rawLng().deg;
  }
  else {
    Raw_Long += (gps.location.rawLng().deg-1);
  }
  Raw_Long = 0 - Raw_Long;
