...
TinyGPSPlus gps;
double my_location[2] = {-71.692500,44.283432};
...
//    Serial.println(String(String(my_location[0],6) + "," + String(my_location[1],6)));
    ...
    long x_coord = getCoord(my_location[0], message_lng);
    long y_coord = getCoord(my_location[1], message_lat);
...
//    avg_size++;
    if(LCD) {
      clearLCD();
      writeLine(String(my_location[0],5), 1, 0);
      writeLine(String(my_location[1],5), 2, 0);
    }
  } else if(LCD) {
    waitScreen();
  }
