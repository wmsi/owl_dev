...
TinyGPSPlus gps;
double my_location[2] = {-71.691528,44.280417};
...
void loop() {
  ...
  if(digitalRead(BUTTON) == LOW) {
    send_string = String(String(my_location[0],6) + "," + String(my_location[1],6));
    connection = radioSend(send_string);
    delay(100);
  }
...
void myLocation() {
  ...
  if(gps.location.isValid()) {
    ...
    if(connection)
      writeLine(String(String(my_location[1],5) + " C"), 2, 0);
    else
