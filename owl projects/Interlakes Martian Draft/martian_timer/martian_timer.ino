/* modified version of ticking_timer.ino. To stop the timer
 *  you have to connect three colored wires correctly.
 *  
...
/*
 * check to see what's up with the input wires
 */
void checkWires() {
  int connection = 0;
  if(analogRead(GREEN) == BLUE) {
    connection++;
  }
  if(analogRead(YELLOW) > BLACK-2 && analogRead(YELLOW) < BLACK+2) {
    connection++;
  }
  if(analogRead(ORANGE) == WHITE) {
    connection++;
  }
  Serial.print(connection); Serial.println(" connections");
}
